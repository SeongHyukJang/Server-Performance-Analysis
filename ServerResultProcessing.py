...
go_data = data['ServerLanguage']['go']
languages = ['Python 3', 'JS (node)', 'Go']

def makeData(resource,method):
    temp_py = py_data[resource][method]
    temp_js = js_data[resource][method]
    temp_go = go_data[resource][method]

    best = list(map(lambda x : float(format(x,'.2f')),[min(temp_py), min(temp_js), min(temp_go)]))
    worst = list(map(lambda x : float(format(x,'.2f')),[max(temp_py), max(temp_js), max(temp_go)]))
    median = list(map(lambda x : float(format(x,'.2f')),[statistics.median(temp_py), statistics.median(temp_js), statistics.median(temp_go)]))
